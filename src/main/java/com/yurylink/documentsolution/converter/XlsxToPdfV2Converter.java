...
import com.yurylink.documentsolution.creator.ExcelUtil;
 ...
 * @author Wesley Yuri
 */
public class XlsxToPdfV2Converter {

	private static final Logger LOGGER = LoggerFactory.getLogger(XlsxToPdfV2Converter.class);
	 ...
	 * @return
	 */
	public XlsxToPdfV2Converter(final ByteArrayInputStream xlsxIS, final Integer columnsQuantity) throws Exception {
	...
	private PdfPCell getPdfCellValueFromXlsxCell(final XSSFCell currentCell) {
		...
		if (currentCell != null) {
			if(currentCell == null ||
					currentCell.getCellType() == XSSFCell.CELL_TYPE_BLANK ||
					(XSSFCell.CELL_TYPE_STRING == currentCell.getCellType() && currentCell.getStringCellValue().isEmpty())){
				cell = new PdfPCell(new Phrase(""));
			}else if (XSSFCell.CELL_TYPE_STRING == currentCell.getCellType()) {

				final Phrase nPhrase =
						new Phrase(
								currentCell.getStringCellValue(),
								ExcelUtil.getFont(
										8,
										currentCell.getCellStyle().getFont().getBold()));
				cell = new PdfPCell(nPhrase);

			} else if (XSSFCell.CELL_TYPE_NUMERIC == currentCell.getCellType()) {
				final Phrase nPhrase =
						new Phrase(
								formatCurrency(currentCell.getNumericCellValue()),
			...
			if(currentCell != null)
				cell.setCellEvent(new XlsxToPdfBorderLineWriter(currentCell.getCellStyle()));
